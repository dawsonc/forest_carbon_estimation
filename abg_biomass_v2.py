...
def abg_biomass_model(group, taxa, spg, df):
   ...
   If the user does not supply a specific gravity, returns a dict returns dictionaries, 
   ...
   matches = 0    
   exact_match = 0                                       # initialization of a variable to check if the user input matches a group and taxa column
   ...
   for i in range(num_rows):
      ...
      if df.iloc[i,0] == group and taxa in taxaCol:     # group/taxa name matches user input
         parameters[(df.iloc[i,0], df.iloc[i,1])] =((df.iloc[i,3],df.iloc[i,4], df.iloc[i,9], df.iloc[i,7]))
         matches += 1
         if spg != None:
            if df.iloc[i,10] <= spg < df.iloc[i,11]:    # spg is in between the upper/lower bound of the group/taxa tree
               exact_match += 1                         # should not have more than one exact match
